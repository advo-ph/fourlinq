...
- **API Hardening:** Email validation on all form endpoints. Status PATCH validates against enum. ID/limit/offset validated and clamped. PATCH returns 404 on missing rows. DB pool with error handler, connection limits, and SIGTERM graceful shutdown. `.env.example` documents all env vars.
