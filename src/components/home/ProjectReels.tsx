...
function ReelCard({
  ...
  reel: (typeof PROJECT_REELS)[number];
  ...
  useEffect(() => {
    ...
    const tryPlay = () => {
      if (!isDisposed) video.play().catch(() => {});
    };
  ...
  return (
    <div className="relative aspect-[5/4] overflow-hidden bg-black/30 rounded-none">
      <video
        ...
        preload="none"
        aria-label={`${reel.location} project video`}
        className="w-full h-full object-cover"
      />
      <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/70 to-transparent pointer-events-none" />
      <div className="pointer-events-none absolute bottom-0 inset-x-0 p-5 lg:p-6">
            ...
            <ul className="flex gap-4 px-4">
              {PROJECT_REELS.map((reel) => (
                <li key={reel.id} className="shrink-0 w-[78vw]">
                  <ReelCard reel={reel} prefersReducedMotion={prefersReducedMotion} />
                </li>
              ))}
            </ul>
          </div>
