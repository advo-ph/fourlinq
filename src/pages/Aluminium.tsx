 ...
 *
 * Data source: cms_aluminium_system table via /api/cms/aluminium. Static
 * fallback below keeps the page rendering if the API is unreachable.
 *
 * Tita can edit / add / remove sub-products from /admin > Content > Aluminium.
 */

interface AluminiumSystem {
...
const FALLBACK_SYSTEMS: AluminiumSystem[] = [
  {
    slug: "thermal-break",
    name: "Thermal Break",
    summary:
...
const Aluminium = () => {
  const query = useQuery({
    ...
    placeholderData: FALLBACK_SYSTEMS,
    staleTime: 5 * 60 * 1000,
    retry: 1,
  });

  const systems = query.error || !query.data?.length ? FALLBACK_SYSTEMS : query.data;
