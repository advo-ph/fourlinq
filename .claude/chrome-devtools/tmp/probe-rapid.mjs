// Per-swipe instrumentation: log active card, cardY/opacity, scrollY, events
// after EVERY swipe at rapid cadence to find which gesture misbehaves.
...
await sleep(120);
console.log("down-1 (+120ms):", JSON.stringify(await state()));
await swipe(client, 500, 620);
await sleep(600);
console.log("down-2 settled:", JSON.stringify(await state()));

// rapid ups: 3 swipes at 160ms cadence with state after each
for (let i = 0; i < 3; i++) {
  await swipe(client, 620, 500);
  await sleep(160);
