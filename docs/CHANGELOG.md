...
- **Projects with a known village are named by place, not by client initial.** "Liloan C. Residence" is now **"Amara, Cebu"**; "Cebu Sch. Residence" is "Monterrazas, Cebu". The name is **derived** from structured `ProjectArea` inside the `projects` export rather than hand-written, so it can never drift from the area data, and every surface — gallery, detail page, site search, admin panel — picked it up without a call-site change. **A village is what earns a derived name:** the other 47 projects keep their residence name ("Cebu M. Residence", "Las Piñas Residence", "Private Residence"), which already carries the location. [src/data/projects.ts](../src/data/projects.ts), [src/data/project-area.ts](../src/data/project-area.ts).
...
- **A confirmed village now outranks `cms_project.title`.** [src/lib/cms-api.ts](../src/lib/cms-api.ts) let a CMS title beat the static name while trusting the static `area` — and production still holds all 61 pre-rename titles that `migrate-cms-data.ts` seeded, so without this the rename would have been a **visual no-op in production**. Scoped to village projects only: the 47 that keep an editorial residence name stay editable from the CMS. The admin `project_name` override still wins over everything.
- **Cards and detail pages show the location once.** For a village project the name is the location, so the eyebrow above it was duplicate text and is gone. On the detail page the Location row appears only when it adds something the heading does not (municipality, e.g. "Liloan"); the `<dl>` divider moved to a position-based rule so the first visible row never carries a stray top border. [src/pages/Inspiration.tsx](../src/pages/Inspiration.tsx), [src/pages/ProjectDetail.tsx](../src/pages/ProjectDetail.tsx).

- **The area filter rail is gone from `/inspiration`.** Two rails stacked above the grid read as two competing ways to narrow the same set, and the area one was redundant: the region headings already announce each area as you scroll. Area is now purely how the page is *organised*, not something the visitor filters by — the category rail (All projects / Windows / Doors / Interior / Exterior) is the only filter, and the region sections now survive a category filter instead of being flattened. `?area=` is no longer read; nothing linked to it. `populatedRegionFilter` existed only to build that rail and was removed with it. [src/pages/Inspiration.tsx](../src/pages/Inspiration.tsx), [src/data/project-area.ts](../src/data/project-area.ts).
- **The catch-all area bucket reads "Other areas"**, not "Area to be confirmed" — that phrasing was an internal to-do showing on a public page, and it promised the visitor a correction we may never make. One constant (`UNKNOWN_REGION_LABEL`) drives both the section heading and the filter chip. [src/data/project-area.ts](../src/data/project-area.ts).

#### Fixed

- **Bare place names, same session.** The first cut of the derivation replaced the name whenever *any* area part was confirmed, which flattened **24 projects** to a bare place: `cebu-maratas-residence` went from "Cebu M. Residence" to "Cebu", and four unrelated houses all ended up called "Cebu". A place without a village does not identify a house. `projectAreaName` now derives only when a village is present, and two tests lock it — one asserting every village project is place-named, one asserting every other project still matches `/Residence/`.
...
- **The residence names still use client initials** ("Cebu M. Residence"), which is the catalog's existing anonymising convention and matches the client's no-addresses note. If she wants those spelled out — "Maratas Residence" — the surnames are in the slugs and it is a data-only change.
