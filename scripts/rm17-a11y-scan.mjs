...
async function cookieLayerScan(browser, finding) {
  for (const viewport of RM17_VIEWPORT) {
    ...
    try {
      ...
      // The Design Tool is embedded on Marvin-hosted pages. Its embed mode must
      // not leak FourLinq's global navigation, footer, chat, or delayed consent
      // layer into that host surface, even in a completely fresh browser state.
      const embedPage = await context.newPage();
      await visitPublicRoute(embedPage, { name: "design-tool-embed", path: "/design-tool?embed=1" }, 1_900);
      const embedState = await embedPage.evaluate(() => ({
        cookie: Boolean(document.querySelector("[data-cookie-banner]")),
        nav: Boolean(document.querySelector("[data-main-nav]")),
        footer: Boolean(document.querySelector("footer")),
        chat: Boolean(document.querySelector("[data-chat-bubble]")),
        tool: Boolean(document.querySelector("[data-design-tool-embed]")),
      }));
      if (!embedState.tool) finding.push(`${viewport.name}: embedded Design Tool hook is missing`);
      if (embedState.cookie) finding.push(`${viewport.name}: cookie consent rendered inside embedded Design Tool`);
      if (embedState.nav) finding.push(`${viewport.name}: global navigation rendered inside embedded Design Tool`);
      if (embedState.footer) finding.push(`${viewport.name}: global footer rendered inside embedded Design Tool`);
      if (embedState.chat) finding.push(`${viewport.name}: chat bubble rendered inside embedded Design Tool`);
    } finally {
      await context.close();
    }
  }
